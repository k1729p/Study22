...
/**
 * The service for the MongoDB database.<br/>
 * <br/>
 * This service uses the 'Active Record' design pattern.<br/>
...
public class AccountMongoService {
	...
	// switch off Sonarqube rule 'static base class members should not be accessed
	// via derived types'
	public void processPayload(String key, String payload) {
		...
		try {
			...
			accountM.persist();
			logger.infof("processPayload(): key[%s], account %s, name[%s]", key, Constants.MONGO_ID_FUN.apply(accountM),
					accountM.name);
		} catch (Exception e) {
