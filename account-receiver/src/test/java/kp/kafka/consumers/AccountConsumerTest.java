...
class AccountConsumerTest {
    ...
    void shouldConsume() throws InterruptedException {
        ...
        final ConsumerRecord<String, String> consumerRecord =
                new ConsumerRecord<>(TOPIC_CONS, 0, 0, RECORD_KEY, RECORD_JSON_PAYLOAD);
        final ConsumerRecords<String, String> consumerRecords = new ConsumerRecords<>(Map.of(
                new TopicPartition(TOPIC_CONS, 0), List.of(consumerRecord)));
    ...
    void shouldNotConsumeRecordWithEmptyRecordKey() throws InterruptedException {
        ...
        // THEN
        Assertions.assertFalse(new CountDownLatch(1).await(WAIT_AFTER_SEND, TimeUnit.MILLISECONDS));
        Mockito.verify(accountMongoService, Mockito.never())
                .processPayload(keyCapture.capture(), payloadCapture.capture());
    }
